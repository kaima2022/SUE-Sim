...
LoadBalancer::CalculateHash (uint32_t destXpuId, uint8_t vcId) const
{
  ...
  switch (m_algorithm)
    ...
    case ENHANCED_HASH:
      {
      // Enhanced hash with optional VC component and optional mixing.
      //
      // Design goals:
      // - Deterministic and cheap (called on every injected transaction)
      // - When useVcInHash=true and NumVcs≈SueCount, different VC IDs should map
      //   to different SUE clients as much as possible (helps rail striping).
      const uint32_t sueCount = static_cast<uint32_t>(m_sueClients.size ());
      // Mix in source (local XPU) so that for a fixed destination the mapping
      // does not collapse to a single rail across all senders (which would
      // create artificial incast onto one rail and can trigger persistent
      // switch-egress drops under lossy overflow modeling).
      uint32_t h = destXpuId ^ ((m_localXpuId + 1) * 0x9e3779b9U);
      h = h * m_hashParams.prime1 + m_hashSeed;
      if (m_hashParams.enableBitOperations)
        {
          // 32-bit mix (Murmur-like finalizer) to spread patterns across SUE count.
          h ^= h >> 16;
          h *= 0x7feb352dU;
          h ^= h >> 15;
          h *= 0x846ca68bU;
      ...
      const uint32_t base = h % sueCount;

      if (m_hashParams.useVcInHash)
        {
          // Use a linear congruential style permutation for VC -> offset.
          // For power-of-two sueCount (common for rail counts), prime2 is odd so
          // the mapping is a permutation over [0, sueCount) when vcId spans it.
          const uint32_t offset =
              (static_cast<uint32_t>(vcId) * m_hashParams.prime2 + m_hashSeed) % sueCount;
          sueId = (base + offset) % sueCount;
        }
      else
        {
          sueId = base;
